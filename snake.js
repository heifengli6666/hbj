...
Snake.prototype = {
	...
	setTimer:function(){
		var speedArr = [500,400,300,200,100,50];
	...
	moveSnake:function(){
		...
		if(in_array(snakeHead,snakeArr) || snakeHead[0]<0 || snakeHead[0]>this.x || snakeHead[1]<0 || snakeHead[1]>this.y){
			...
			alert('GAME OVER！本次得分：'+this.score);
			this.init();
			return;
		}
		 ...
		 if(snakeHead.toString() != this.coolPoint.toString()){
			...
			this.speed =  Math.ceil((this.score + 1)/80);
			this.speedele.innerHTML="难度星数："+this.speed;
		}
	...
	drawCell:function(pos,type){//绘制会用到的几种颜色的图。
		...
		var colorArr = ['#FFFFF4','#9AFF02',"#4DFFFF"];
		var cxt = this.cxt;
		var area;
